...
module.exports = {
    ...
    ironmanPage: async (req, res) => {
        ...
        if(!isNaN(page)) {
            ...
            if(parseInt(pageCount) > 1) {
                res.render('hiscores/ironman', {rows: rows, output: output, next: parseInt(currentPage) + 1, prev: parseInt(currentPage) - 1});
            } else {
    ...
    ironmanSkillPage: async (req, res) => {
        ...
        if(!isNaN(id) && id < 26 && id > -1) {
            ...
            if(parseInt(pageCount) > 1) {
                res.render('hiscores/ironskill', {rows: rows, id: id, output: output, skillName: SKILLS[id], next: parseInt(currentPage) + 1, prev: parseInt(currentPage) - 1});
            } else {
    ...
    hcimPage: async (req, res) => {
        ...
        if(!isNaN(page)) {
            ...
            if(parseInt(pageCount) > 1) {
                res.render('hiscores/hcim', {rows: rows, output: output, next: parseInt(currentPage) + 1, prev: parseInt(currentPage) - 1});
            } else {
    ...
    regularGetPage: async (req, res) => {
        ...
        if(parseInt(pageCount) > 1) {
            ...
            res.render('hiscores/index', {output: output, next: parseInt(currentPage) + 1, prev: parseInt(currentPage) - 1, rows: rows});
        } else {
            let rows = await database.query(q2);
            for(var i = 0; i < rows.length; i++) {
                let username = await database.query(q3, [rows[i].userID]);
